...
exports.transferUSDC = exports.createSmartAccount = void 0;
...
const transferUSDC = (amount, receipient) => __awaiter(void 0, void 0, void 0, function* () {
    // amount to approve and transfer
    const transferAmount = amount;
    const fee = transferAmount * 11 / 1000;
    console.log(fee);
    const totalApproveAmount = transferAmount + fee;
    const amountToTransfer = (0, viem_1.parseUnits)(transferAmount.toString(), 6);
    const approvalAmount = (0, viem_1.parseUnits)(totalApproveAmount.toString(), 6);
    // call the create smart contract function to get the smart account address
    const { smartWallet, saAddress } = yield (0, exports.createSmartAccount)(process.env.PRIVATE_KEY);
    // call the gasless transfer to smart account function  
    // check gasless transfer to smart account transaction status 
    const res = yield (0, arc_gasless_1.checkTransactionStatus)(saAddress, transferAmount, process.env.PRIVATE_KEY);
    console.log(res);
    if (!(res && res.data && res.data.txStatus === "CONFIRMED")) {
        ...
        const usdcAbi = (0, viem_1.parseAbi)(["function approve(address spender, uint256 amount)"]);
        const zydeAbi = (0, viem_1.parseAbi)(["function transferUSDC(address _recipient, uint256 _amount)"]);
        const usdcData = (0, viem_1.encodeFunctionData)({
            abi: usdcAbi,
            functionName: "approve",
            args: [zydeContractAddress, approvalAmount],
        });
        const zydeData = (0, viem_1.encodeFunctionData)({
            abi: zydeAbi,
            functionName: "transferUSDC",
            args: [receipient, amountToTransfer],
        });
